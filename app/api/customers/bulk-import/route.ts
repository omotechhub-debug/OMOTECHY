  ...
  try {
    ...
    for (let i = 0; i < customers.length; i++) {
      ...
      try {
        ...
        const normalizedPhone = normalizeKenyaPhoneLocal(cleanPhone);
        if (!normalizedPhone) {
          results.errors.push({
            row: i + 1,
            error: 'Invalid phone (expected Kenyan mobile e.g. 07XXXXXXXX)',
            data: customerData
          });
          results.skipped++;
          continue;
        ...
        if (existingCustomer) {
          results.errors.push({
            row: i + 1,
            error: `Customer already exists: ${customerData.name} (${normalizedPhone}) - Exact name and phone match found`,
            data: customerData
          });
          results.skipped++;
          continue;
