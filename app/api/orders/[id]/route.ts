...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (finalUpdateData.customer && typeof finalUpdateData.customer.phone === 'string') {
      const np = normalizeKenyaPhoneLocal(finalUpdateData.customer.phone);
      if (!np) {
        return NextResponse.json(
          {
            success: false,
      ...
      finalUpdateData.customer = { ...finalUpdateData.customer, phone: np };
    }
