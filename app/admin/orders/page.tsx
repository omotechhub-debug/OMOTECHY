...
import { resolvePhoneFromOrderFields } from '@/lib/phone-utils';
...
function OrdersPageContent() {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        toast({
          ...
          description:
            n === 0
              ? 'No pending orders were found to update.'
              : `${n} pending order${n === 1 ? '' : 's'} confirmed.`,
        });
  ...
  const handleInitiatePayment = (order: Order) => {
    ...
    setPaymentPhone(
      resolvePhoneFromOrderFields(order as any) || order.customer.phone || ''
    );
    setPaymentType('full');
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {pendingOrdersCount > 0 && (
            <Button
              variant="outline"
              onClick={() => setAcceptAllDialogOpen(true)}
              disabled={updating || acceptingAll}
              className="flex items-center gap-2 border-emerald-300 text-emerald-700 hover:bg-emerald-50"
            >
              <CheckCircle className="w-4 h-4" />
              Accept all pending
              <span className="text-xs font-normal text-emerald-600/80">({pendingOrdersCount})</span>
            </Button>
          )}
        ...
        <div className="flex items-center justify-between bg-white rounded-lg shadow-lg p-4 border border-gray-200">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-wrap items-center gap-1 max-w-full">
              {getPaginationPageItems(currentPage, totalPages).map((item, idx) =>
                item === 'ellipsis' ? (
                  <span
                    key={`ellipsis-${idx}`}
                    className="flex h-8 min-w-8 items-center justify-center px-1 text-sm text-gray-400"
                    aria-hidden
                  >
                    …
                  </span>
                  ...
                  <Button
                    ...
                    className={`w-8 h-8 shrink-0 p-0 ${
                      currentPage === item
      ...
      {/* Accept all pending confirmation */}
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-600">
              Confirms all pending orders you can manage
              {user?.role === 'manager' ? ' (your station)' : ' (every station)'}.
              About {pendingOrdersCount} pending {pendingOrdersCount === 1 ? 'is' : 'are'} shown in this list; the number
              updated is reported when done. SMS is not sent for bulk accept—use per-order Accept for notifications.
            </DialogDescription>
          </DialogHeader>
