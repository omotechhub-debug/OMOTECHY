...
export default function MpesaTransactionsPage() {
      ...
      transactions.filter((transaction) => {
        ...
        const matchesStatus =
          ...
          (statusFilter === 'connected' &&
            ...
            getConnectedOrder(transaction)) ||
          (statusFilter === 'unconnected' && !transaction.isConnectedToOrder) ||
          (statusFilter === 'missing-order' &&
            transaction.isConnectedToOrder &&
            !getConnectedOrder(transaction));

        return matchesSearch && matchesStatus;
      }),
    [transactions, searchTerm, statusFilter, orders]
  );
  ...
  const indexOfLast = page * transactionsPerPage;
  const indexOfFirst = indexOfLast - transactionsPerPage;
  const paginatedTransactions = filteredTransactions.slice(indexOfFirst, indexOfLast);
          ...
          <CardDescription>
            ...
            {filteredTransactions.length > transactionsPerPage && (
              <span className="block mt-1 text-xs text-muted-foreground">
                Showing {indexOfFirst + 1}–{Math.min(indexOfLast, filteredTransactions.length)} of{' '}
