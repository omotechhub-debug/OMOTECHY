...
export default function ClientsPage() {
  ...
  async function repairOrderPhonesInDb() {
    setRepairingPhones(true)
    setError("")
    try {
      const response = await fetch("/api/admin/repair-order-phones", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
      ...
      setError("Repair failed. Try again.")
    } finally {
      setRepairingPhones(false)
    }
  }
  ...
  function validateClientData(clientData: any) {
    ...
    const np = normalizeKenyaPhoneLocal(clientData.phone)
    if (!clientData.phone?.trim()) {
      setError("Phone is required")
      return false
    }
    if (!np) {
      setError("Use Kenyan mobile format: 10 digits starting with 07 (e.g. 0712345678)")
      return false
    }
    
    // Check for duplicate phone number
    if (clients.some(c => normalizeKenyaPhoneLocal(c.phone) === np)) {
  ...
  function handleEditClient(e: any) {
    ...
    const editNorm = normalizeKenyaPhoneLocal(tempClientData.phone)
    if (!tempClientData.phone?.trim()) {
      setError("Phone is required")
      setEditing(false)
      return
    }
    if (!editNorm) {
      setError("Use Kenyan mobile format: 10 digits starting with 07")
      setEditing(false)
      return
    }
    
    // Check for duplicate phone number
    if (otherClients.some(c => normalizeKenyaPhoneLocal(c.phone) === editNorm)) {
      setError("A client with this phone number already exists")
      setEditing(false)
      return
    }
    ...
    fetch(`/api/customers/${editingClient.id}`, {
      ...
      body: JSON.stringify({ ...editForm, phone: editNorm }),
    })
    .then(response => response.json())
    .then(data => {
      if (data.success) {
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex gap-3 mt-4 md:mt-0">
          ...
          <Button
            ...
            onClick={repairOrderPhonesInDb}
            disabled={repairingPhones}
            title="Rewrite bad order customer phones from STK prompt / valid order fields (admin)"
          >
            ...
            <div className="flex justify-center mt-8">
              <div className="flex flex-wrap items-center justify-center gap-2 max-w-full">
                ...
                <div className="flex flex-wrap items-center justify-center gap-1 max-w-full">
                  {getPaginationPageItems(page, totalPages).map((item, idx) =>
                    item === "ellipsis" ? (
                      <span
                        key={`ellipsis-${idx}`}
                      ...
                      <Button
                        ...
                        className={`h-9 w-9 shrink-0 rounded-xl p-0 ${
                          page === item ? "bg-primary hover:bg-primary/90 text-white" : ""
                        }`}
        ...
        <DialogContent className="max-w-md w-full p-0 overflow-hidden bg-gradient-to-br from-white via-secondary to-accent/10 rounded-2xl shadow-2xl">
          <div className="flex flex-col items-center p-6 luxury-card bg-transparent">
            ...
            <form onSubmit={async e => {
              ...
              const addNorm = normalizeKenyaPhoneLocal(newClient.phone)!
              try {
                const response = await fetch('/api/customers', {
                  ...
                  body: JSON.stringify({ ...newClient, phone: addNorm }),
                })
                  ...
                  {filteredClients.map((client) => (
                    <div
                      ...
                      className={`flex items-center gap-3 p-2 rounded hover:bg-gray-50 ${
                        !canMessagePhone(client.phone) ? 'opacity-50' : ''
                      }`}
                    >
                      <Checkbox
                        checked={selectedClients.includes(client.id)}
